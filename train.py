...
class SequencePredictor():
    ...
    def add_loss_op(self, preds):
        # start_pred = preds[0]
        # end_pred = preds[1]
        # loss_1 = tf.nn.softmax_cross_entropy_with_logits(start_pred, self.labels_placeholder)
        # loss_2 = tf.nn.softmax_cross_entropy_with_logits(end_pred, self.labels_placeholder)
    ...
    def output(self, sess):
        batch_preds = []
        for inputs,labels, masks, sequences in batch_generator(self.embedding_wrapper, self.dev_data_file, self.dev_indices_data_file, self.dev_sequence_data_file, self.batch_size, self.bill_length):
            preds_ = self.predict_on_batch(sess, inputs, labels, masks, sequences)
            batch_preds += list(preds_)
        # print "output preds: ", batch_preds
        return batch_preds
    ...
    def evaluate(self, sess):
        ...
        for batch_preds in self.output(sess):
            ...
            for preds in batch_preds:
                ...
                # print "evalu preds: ", preds
                # print
                max1, max2 = heapq.nlargest(2, preds)
                index_max1, index_max2 = np.argsort(preds)[-2:]
                prediction = set()
    ...
    def predict_on_batch(self, sess, inputs_batch, labels_batch, mask_batch, sequence_batch):
        feed = self.create_feed_dict(inputs_batch = inputs_batch, labels_batch=labels_batch, masks_batch=mask_batch, sequences = sequence_batch)
    ...
    def run_epoch(self, sess):
        for inputs,labels, masks, sequences in batch_generator(self.embedding_wrapper, self.train_data_file, self.train_indices_data_file, self.train_sequence_data_file, self.batch_size, self.bill_length):
            loss = self.train_on_batch(sess, inputs, labels, masks, sequences)
        print("")
